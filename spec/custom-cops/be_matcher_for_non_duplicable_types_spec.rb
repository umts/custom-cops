...
describe RuboCop::Cop::UmtsCustomCops::BeMatcherForNonDuplicableTypes do
  ...
  context 'failure cases' do
    ...
    it('finds an expectation with eq/true')  { inspect_source cop, 'expect(stuff).to eq true' }
    it('finds an expectation with eql/true') { inspect_source cop, 'expect(stuff).to eql true' }
  ...
  context 'non-failure cases' do
    ...
    it('skips nodes with other types')     { inspect_source cop, "expect(stuff).to be 'a string'" }
  end
